...
interface SessionAppClient {
  api: {
    auth: {
      "get-session": {
        $get: () => Promise<{ ok: boolean; json: () => Promise<unknown> }>;
      };
      "sign-out": { $post: () => Promise<unknown> };
    };
  };
}

export function createSessionStore(appClient: SessionAppClient) {
